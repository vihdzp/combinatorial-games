...
* Lemmas of the form `(∀ y ∈ moves p (f x), P y) ↔ _` and analogous, as long as any quantifiers
  in the simplified form are over left or right moves of simpler games.
* Lemmas of the form `moves p (f x) = _` and analogous, as long as the simplified set is of the
