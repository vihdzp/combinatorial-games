...
quantifier-less way. For instance, `∀ y ∈ moves_left !{{0, 1} | {2, 3}}, P y` can be simplified into
`P 0 ∧ P 1`.
...
* Lemmas of the form `(∀ y ∈ moves_left (f x), P y) ↔ _` and analogous, as long as any quantifiers
  in the simplified form are over left or right moves of simpler games.
* Lemmas of the form `moves_left (f x) = _` and analogous, as long as the simplified set is of the
