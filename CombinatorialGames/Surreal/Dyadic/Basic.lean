...
theorem eq_lower_of_mem_moves_left_toIGame {x : Dyadic} {y : IGame} (h : y ∈ moves_left x) :
    y = lower x := by
  by_cases hx : x.den = 1
...
theorem eq_upper_of_mem_moves_right_toIGame {x : Dyadic} {y : IGame} (h : y ∈ moves_right x) :
    y = upper x := by
  have : -y ∈ moves_left (-x : Dyadic) := by simpa
...
theorem toIGame_add_equiv (x y : Dyadic) : ((x + y : Dyadic) : IGame.{u}) ≈ x + y := by
  ...
  · intro z hz
    ...
    · by_cases hy : y.den = 1; simp_all
      ...
      have : (lower y : IGame) ∈ moves_left y := by rw [hy]; simp
    ...
    · use lower x + y
      ...
      have : (lower x : IGame) ∈ moves_left x := by rw [hx]; simp
  ...
  · intro z hz
    ...
    · by_cases hy : y.den = 1; simp_all
      use x + upper y
      have hy := toIGame_of_den_ne_one hy
      have : (upper y : IGame) ∈ moves_right y := by rw [hy]; simp
      rw [← (toIGame_add_equiv ..).le_congr_left, hy]
      simpa using le_upper_add_of_den_le h
    · use upper x + y
      have hx := toIGame_of_den_ne_one (den_ne_one_of_den_lt h)
      have : (upper x : IGame) ∈ moves_right x := by rw [hx]; simp
...
private theorem equiv_dyadic (x : IGame) [Short x] [Numeric x] : ∃ y : Dyadic, x ≈ y.toIGame := by
  have H₁ (y : x.moves_left) : ∃ z : Dyadic, y.1 ≈ z.toIGame := by
    have := Numeric.of_mem_moves y.2
    have := Short.of_mem_moves y.2
    exact IGame.equiv_dyadic _
  have H₂ (y : x.moves_right) : ∃ z : Dyadic, y.1 ≈ z.toIGame := by
    have := Numeric.of_mem_moves y.2
    have := Short.of_mem_moves y.2
    exact IGame.equiv_dyadic _
  choose f hf using H₁
