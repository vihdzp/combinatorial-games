...
form `y ∈ x.moves_left` or `y ∈ x.moves_right`, which prove termination. Alternatively, you can use
...
abbrev moves_left (x : IGame.{u}) : Set IGame.{u} := x.moves left

/-- The set of right moves of the game. -/
abbrev moves_right (x : IGame.{u}) : Set IGame.{u} := x.moves right
...
@[simp, game_cmp]
theorem moves_left_ofSets (s t : Set IGame) [Small.{u} s] [Small.{u} t] : !{s | t}.moves_left = s :=
  moves_ofSets ..

@[simp, game_cmp]
theorem moves_right_ofSets (s t : Set IGame) [Small.{u} s] [Small.{u} t] : !{s | t}.moves_right = t :=
  moves_ofSets ..

@[simp]
theorem ofSets_moves_left_moves_right (x : IGame) : !{x.moves_left | x.moves_right} = x := by
...
lemma isOption_iff_mem_union {x y : IGame} :
    IsOption x y ↔ x ∈ y.moves_left ∪ y.moves_right := by
...
def ofSetsRecOn {motive : IGame.{u} → Sort*} (x)
    ...
    motive x :=
  cast (by simp) <| moveRecOn (motive := fun x ↦ motive !{x.moves_left | x.moves_right}) x
    fun x IH ↦ mk _ _
...
@[simp, game_cmp] theorem moves_left_zero : moves_left 0 = ∅ := moves_ofSets ..
@[simp, game_cmp] theorem moves_right_zero : moves_right 0 = ∅ := moves_ofSets ..
...
@[simp, game_cmp] theorem moves_left_one : moves_left 1 = {0} := moves_left_ofSets ..
@[simp, game_cmp] theorem moves_right_one : moves_right 1 = ∅ := moves_right_ofSets ..
...
instance : LE IGame where
  le := Sym2.GameAdd.fix isOption_wf fun x y le ↦
    (∀ z (h : z ∈ x.moves_left),  ¬le y z (Sym2.GameAdd.snd_fst (IsOption.of_mem_moves h))) ∧
    (∀ z (h : z ∈ y.moves_right), ¬le z x (Sym2.GameAdd.fst_snd (IsOption.of_mem_moves h)))
...
theorem le_iff_forall_lf {x y : IGame} :
    x ≤ y ↔ (∀ z ∈ x.moves_left, z ⧏ y) ∧ (∀ z ∈ y.moves_right, x ⧏ z) :=
  propext_iff.1 <| Sym2.GameAdd.fix_eq ..

/-- Definition of `x ⧏ y` on games, in terms of `≤`. -/
theorem lf_iff_exists_le {x y : IGame} :
    x ⧏ y ↔ (∃ z ∈ y.moves_left, x ≤ z) ∨ (∃ z ∈ x.moves_right, z ≤ y) := by
  simpa [not_and_or, -not_and] using le_iff_forall_lf.not

/-- The definition of `0 ≤ x` on games, in terms of `0 ⧏`. -/
theorem zero_le {x : IGame} : 0 ≤ x ↔ ∀ y ∈ x.moves_right, 0 ⧏ y := by
  rw [le_iff_forall_lf]; simp

/-- The definition of `x ≤ 0` on games, in terms of `⧏ 0`. -/
theorem le_zero {x : IGame} : x ≤ 0 ↔ ∀ y ∈ x.moves_left, y ⧏ 0 := by
  rw [le_iff_forall_lf]; simp

/-- The definition of `0 ⧏ x` on games, in terms of `0 ≤`. -/
theorem zero_lf {x : IGame} : 0 ⧏ x ↔ ∃ y ∈ x.moves_left, 0 ≤ y := by
  rw [lf_iff_exists_le]; simp

/-- The definition of `x ⧏ 0` on games, in terms of `≤ 0`. -/
theorem lf_zero {x : IGame} : x ⧏ 0 ↔ ∃ y ∈ x.moves_right, y ≤ 0 := by
  rw [lf_iff_exists_le]; simp
...
theorem le_def {x y : IGame} : x ≤ y ↔
    (∀ a ∈ x.moves_left,  (∃ b ∈ y.moves_left, a ≤ b) ∨ (∃ b ∈ a.moves_right, b ≤ y)) ∧
    (∀ a ∈ y.moves_right, (∃ b ∈ a.moves_left, x ≤ b) ∨ (∃ b ∈ x.moves_right, b ≤ a)) := by
  rw [le_iff_forall_lf]
...
theorem lf_def {x y : IGame} : x ⧏ y ↔
    (∃ a ∈ y.moves_left,  (∀ b ∈ x.moves_left, b ⧏ a) ∧ (∀ b ∈ a.moves_right, x ⧏ b)) ∨
    (∃ a ∈ x.moves_right, (∀ b ∈ a.moves_left, b ⧏ y) ∧ (∀ b ∈ y.moves_right, a ⧏ b)) := by
  rw [lf_iff_exists_le]
  congr! <;> rw [le_iff_forall_lf]

theorem left_lf_of_le {x y z : IGame} (h : x ≤ y) (h' : z ∈ x.moves_left) : z ⧏ y :=
  (le_iff_forall_lf.1 h).1 z h'

theorem lf_right_of_le {x y z : IGame} (h : x ≤ y) (h' : z ∈ y.moves_right) : x ⧏ z :=
  (le_iff_forall_lf.1 h).2 z h'

theorem lf_of_le_left {x y z : IGame} (h : x ≤ z) (h' : z ∈ y.moves_left) : x ⧏ y :=
  lf_iff_exists_le.2 <| Or.inl ⟨z, h', h⟩

theorem lf_of_right_le {x y z : IGame} (h : z ≤ y) (h' : z ∈ x.moves_right) : x ⧏ y :=
...
theorem left_lf {x y : IGame} (h : y ∈ x.moves_left) : y ⧏ x :=
  lf_of_le_left le_rfl h

theorem lf_right {x y : IGame} (h : y ∈ x.moves_right) : x ⧏ y :=
...
theorem equiv_of_forall_lf {x y : IGame}
    (hl₁ : ∀ a ∈ x.moves_left,  ¬y ≤ a)
    (hr₁ : ∀ a ∈ x.moves_right, ¬a ≤ y)
    (hl₂ : ∀ b ∈ y.moves_left,  ¬x ≤ b)
    (hr₂ : ∀ b ∈ y.moves_right, ¬b ≤ x) : x ≈ y := by
  constructor <;> refine le_iff_forall_lf.2 ⟨?_, ?_⟩ <;> assumption

theorem equiv_of_exists_le {x y : IGame}
    (hl₁ : ∀ a ∈ x.moves_left,  ∃ b ∈ y.moves_left,  a ≤ b)
    (hr₁ : ∀ a ∈ x.moves_right, ∃ b ∈ y.moves_right, b ≤ a)
    (hl₂ : ∀ b ∈ y.moves_left,  ∃ a ∈ x.moves_left,  b ≤ a)
    (hr₂ : ∀ b ∈ y.moves_right, ∃ a ∈ x.moves_right, a ≤ b) : x ≈ y := by
  apply equiv_of_forall_lf <;> simp +contextual [hl₁, hl₂, hr₁, hr₂, lf_iff_exists_le]

theorem equiv_of_exists {x y : IGame}
    (hl₁ : ∀ a ∈ x.moves_left,  ∃ b ∈ y.moves_left,  a ≈ b)
    (hr₁ : ∀ a ∈ x.moves_right, ∃ b ∈ y.moves_right, a ≈ b)
    (hl₂ : ∀ b ∈ y.moves_left,  ∃ a ∈ x.moves_left,  a ≈ b)
    (hr₂ : ∀ b ∈ y.moves_right, ∃ a ∈ x.moves_right, a ≈ b) : x ≈ y := by
...
private def neg' (x : IGame) : IGame :=
  !{range fun y : x.moves_right ↦ neg' y.1 | range fun y : x.moves_left ↦ neg' y.1}
termination_by x
decreasing_by igame_wf
...
theorem neg_eq (x : IGame) : -x = !{-x.moves_right | -x.moves_left} := by
...
private def add' (x y : IGame) : IGame :=
  !{(range fun z : x.moves_left ↦ add' z y) ∪ (range fun z : y.moves_left ↦ add' x z) |
    (range fun z : x.moves_right ↦ add' z y) ∪ (range fun z : y.moves_right ↦ add' x z)}
termination_by (x, y)
decreasing_by igame_wf
...
theorem add_eq (x y : IGame) : x + y =
    !{(· + y) '' x.moves_left ∪ (x + ·) '' y.moves_left |
      (· + y) '' x.moves_right ∪ (x + ·) '' y.moves_right} := by
  change add' _ _ = _
  rw [add']
...
private theorem sub_self_le (x : IGame) : x - x ≤ 0 := by
  rw [le_zero, moves_left, moves_sub]
...
private theorem add_le_add_left' {x y : IGame} (h : x ≤ y) (z : IGame) : z + x ≤ z + y := by
  rw [le_iff_forall_lf, moves_left, moves_right, moves_add, moves_add]
...
@[game_cmp]
theorem moves_left_natCast_succ' : ∀ n : ℕ, moves_left n.succ = {(n : IGame)}
  | 0 => by simp
  | n + 1 => by
    rw [Nat.cast_succ, moves_left, moves_add, ← moves_left, moves_left_natCast_succ']
    simp

@[simp 1100] -- This should trigger before `moves_left_add`.
theorem moves_left_natCast_succ (n : ℕ) : moves_left (n + 1) = {(n : IGame)} :=
  moves_left_natCast_succ' n

@[simp 1100, game_cmp] -- This should trigger before `moves_right_add`.
theorem moves_right_natCast : ∀ n : ℕ, moves_right n = ∅
  | 0 => by simp
  | n + 1 => by
    rw [Nat.cast_succ, moves_right, moves_add, ← moves_right, moves_right_natCast]
    simp

@[simp 1100, game_cmp]
theorem moves_left_ofNat (n : ℕ) [n.AtLeastTwo] : moves_left ofNat(n) = {((n - 1 : ℕ) : IGame)} := by
  change moves_left n = _
  rw [← Nat.succ_pred (NeZero.out (n := n)), moves_left_natCast_succ']
  simp

@[simp 1100, game_cmp]
theorem moves_right_ofNat (n : ℕ) [n.AtLeastTwo] : moves_right ofNat(n) = ∅ :=
...
theorem eq_natCast_of_mem_moves_left_natCast {n : ℕ} {x : IGame} (hx : x ∈ moves_left n) :
    ∃ m : ℕ, m < n ∧ m = x := by
  cases n with
  | zero => simp at hx
  | succ n =>
...
theorem eq_sub_one_of_mem_moves_left_intCast {n : ℤ} {x : IGame} (hx : x ∈ moves_left n) :
    x = (n - 1 : ℤ) := by
...
theorem eq_add_one_of_mem_moves_right_intCast {n : ℤ} {x : IGame} (hx : x ∈ moves_right n) :
    x = (n + 1 : ℤ) := by
  have : -x ∈ moves_left (-n : ℤ) := by simpa
  rw [← neg_inj]
...
theorem eq_intCast_of_mem_moves_left_intCast {n : ℤ} {x : IGame} (hx : x ∈ moves_left n) :
    ∃ m : ℤ, m < n ∧ m = x := by
  use n - 1
  simp [eq_sub_one_of_mem_moves_left_intCast hx]

/-- Every right option of an integer is equal to a larger integer. -/
theorem eq_intCast_of_mem_moves_right_intCast {n : ℤ} {x : IGame} (hx : x ∈ moves_right n) :
    ∃ m : ℤ, n < m ∧ m = x := by
  use n + 1
...
def mul' (x y : IGame) : IGame :=
  !{(range fun a : (x.moves_left ×ˢ y.moves_left ∪ x.moves_right ×ˢ y.moves_right :) ↦
    mul' a.1.1 y + mul' x a.1.2 - mul' a.1.1 a.1.2) |
  (range fun a : (x.moves_left ×ˢ y.moves_right ∪ x.moves_right ×ˢ y.moves_left :) ↦
    mul' a.1.1 y + mul' x a.1.2 - mul' a.1.1 a.1.2)}
termination_by (x, y)
...
theorem mul_eq (x y : IGame) : x * y =
    !{(fun a ↦ mulOption x y a.1 a.2) ''
      (x.moves_left ×ˢ y.moves_left ∪ x.moves_right ×ˢ y.moves_right) |
    (fun a ↦ mulOption x y a.1 a.2) ''
      (x.moves_left ×ˢ y.moves_right ∪ x.moves_right ×ˢ y.moves_left)} := by
  change mul' _ _ = _
  rw [mul']
...
theorem zero_mem_moves_left_inv {x : IGame} (hx : 0 < x) : 0 ∈ x⁻¹.moves_left := by
  rw [inv_eq hx, moves_left_ofSets]
  exact ⟨InvTy.zero, rfl⟩

theorem inv_nonneg {x : IGame} (hx : 0 < x) : 0 ⧏ x⁻¹ :=
  left_lf (zero_mem_moves_left_inv hx)
