...
instance : DecidableEq FGame
  | s₁, s₂ => Quotient.recOnSubsingleton₂ s₁ s₂ fun _ _ ↦ decidable_of_iff' (_ ≡ _) Quotient.eq_iff_equiv
...
theorem Subposition.single {x y : FGame} (h : IsOption x y) : Subposition x y :=
...
@[simp]
theorem _root_.Finset.image_attach_comp {α β : Type*} [DecidableEq β] (s : Finset α) (f : α → β) :
    s.attach.image (f ∘ Subtype.val) = s.image f := by
  classical
  rw [← Finset.image_image, Finset.attach_image_val]

@[simp]
theorem _root_.Finset.image_attach {α β : Type*} [DecidableEq β] (s : Finset α) (f : α → β) :
...
example : (1 : FGame.{u}) + 0 = (1 : FGame.{u}) := by decide
