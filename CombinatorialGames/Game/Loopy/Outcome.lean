...
mutual
  ...
  inductive IsLeftWin : LGame → Prop where
    | intro (x y : LGame) : y ∈ x.moves_left → IsRightLoss y → IsLeftWin x
  /-- `IsRightLoss x` means that right loses `x` going first. -/
  inductive IsRightLoss : LGame → Prop where
    | intro (x : LGame) : (∀ y ∈ x.moves_right, IsLeftWin y) → IsRightLoss x
end

mutual
  /-- `IsRightWin x` means that right wins `x` going first. -/
  inductive IsRightWin : LGame → Prop where
    | intro (x y : LGame) : y ∈ x.moves_right → IsLeftLoss y → IsRightWin x
  /-- `IsLeftLoss x` means that left loses `x` going first. -/
  inductive IsLeftLoss : LGame → Prop where
    | intro (x : LGame) : (∀ y ∈ x.moves_left, IsRightWin y) → IsLeftLoss x
end

theorem isLeftWin_iff_exists {x : LGame} : IsLeftWin x ↔ ∃ y ∈ x.moves_left, IsRightLoss y where
  mp h := h.rec (motive_2 := fun _ _ ↦ True) (fun _x y hyx hy _ ↦ ⟨y, hyx, hy⟩) fun _ _ _ ↦ ⟨⟩
  mpr := fun ⟨y, hyx, hy⟩ ↦ .intro x y hyx hy

theorem isRightWin_iff_exists {x : LGame} : IsRightWin x ↔ ∃ y ∈ x.moves_right, IsLeftLoss y where
  mp h := h.rec (motive_2 := fun _ _ ↦ True) (fun _x y hyx hy _ ↦ ⟨y, hyx, hy⟩) fun _ _ _ ↦ ⟨⟩
  mpr := fun ⟨y, hyx, hy⟩ ↦ .intro x y hyx hy

theorem isLeftLoss_iff_forall {x : LGame} : IsLeftLoss x ↔ ∀ y ∈ x.moves_left, IsRightWin y where
  mp h := h.rec (motive_1 := fun _ _ ↦ True) (fun _ _ _ _ _ ↦ ⟨⟩) fun _ h _ ↦ h
  mpr := .intro x

theorem isRightLoss_iff_forall {x : LGame} : IsRightLoss x ↔ ∀ y ∈ x.moves_right, IsLeftWin y where
  mp h := h.rec (motive_1 := fun _ _ ↦ True) (fun _ _ _ _ _ ↦ ⟨⟩) fun _ h _ ↦ h
  mpr := .intro x

theorem not_isLeftWin_iff_forall {x : LGame} :
    ¬ IsLeftWin x ↔ ∀ y ∈ x.moves_left, ¬ IsRightLoss y := by
  simp [isLeftWin_iff_exists]

theorem not_isRightWin_iff_forall {x : LGame} :
    ¬ IsRightWin x ↔ ∀ y ∈ x.moves_right, ¬ IsLeftLoss y := by
  simp [isRightWin_iff_exists]

theorem not_isLeftLoss_iff_exists {x : LGame} :
    ¬ IsLeftLoss x ↔ ∃ y ∈ x.moves_left, ¬ IsRightWin y := by
  simp [isLeftLoss_iff_forall]

theorem not_isRightLoss_iff_exists {x : LGame} :
    ¬ IsRightLoss x ↔ ∃ y ∈ x.moves_right, ¬ IsLeftWin y := by
...
def IsLeftStrategy (s : Set LGame) : Prop :=
  ∀ y ∈ s, ∀ z ∈ y.moves_right, ∃ r ∈ z.moves_left, r ∈ s
...
def IsRightStrategy (s : Set LGame) : Prop :=
  ∀ y ∈ s, ∀ z ∈ y.moves_left, ∃ r ∈ z.moves_right, r ∈ s
...
-- Note: the spelling `Relation.Comp (· ∈ ·.moves_right) (· ∈ ·.moves_left)` is slightly longer
theorem not_isLeftDraw_of_acc_comp {x : LGame}
    (h : Acc (fun z x ↦ ∃ y ∈ x.moves_left, z ∈ y.moves_right) x) : ¬ IsLeftDraw x :=
...
theorem not_isRightDraw_of_acc_comp {x : LGame}
    (h : Acc (fun z x ↦ ∃ y ∈ x.moves_right, z ∈ y.moves_left) x) : ¬ IsRightDraw x :=
  h.rec fun _x _ ih ⟨nw, nl⟩ ↦
...
private theorem win_loss_iff {x : IGame} :
  ...
  x.moveRecOn fun x h ↦ by
    rw [isLeftWin_iff_exists, isLeftLoss_iff_forall, isRightWin_iff_exists, isRightLoss_iff_forall,
      zero_lf, lf_zero, le_zero, zero_le, LGame.moves_left, LGame.moves_right, moves_toLGame,
      moves_toLGame]
