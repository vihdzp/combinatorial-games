...
theorem QPF.Cofix.unique {F : Type u → Type u} [QPF F] {α : Type u}
  ...
  ext x
  refine QPF.Cofix.bisim_rel
    (fun a b => ∃ x, f x = a ∧ g x = b)
    (fun a b hab => ?_)
    (f x) (g x) ⟨x, rfl, rfl⟩
  obtain ⟨x, rfl, rfl⟩ := hab
  simp only [funext_iff, Function.comp_apply] at hf hg
...
private def F : Type (u + 1) → Type (u + 1) :=
  fun X => { s : Set X × Set X // Small.{u} s.1 ∧ Small.{u} s.2 }

private local instance {α} (x : F.{u} α) : Small.{u} x.1.1 := x.2.1
private local instance {α} (x : F.{u} α) : Small.{u} x.1.2 := x.2.2

private noncomputable local instance : QPF.{u + 1, u + 1, u + 1} F where
  map f s := ⟨(f '' s.1.1, f '' s.1.2), inferInstance, inferInstance⟩
  P := {
    A := Type u × Type u
    B c := PLift c.1 ⊕ PLift c.2
  }
  abs x := ⟨(Set.range (x.2 ∘ Sum.inl), Set.range (x.2 ∘ Sum.inr)), inferInstance, inferInstance⟩
  repr x := ⟨(Shrink x.1.1, Shrink x.1.2),
    (Equiv.sumArrowEquivProdArrow _ _ _).symm
    (fun i => ((equivShrink _).symm i.down :),
    fun i => ((equivShrink _).symm i.down :))⟩
  abs_repr _ := Subtype.ext (Prod.ext
    (((Equiv.plift.trans (equivShrink _).symm).surjective.range_comp _).trans Subtype.range_coe)
    (((Equiv.plift.trans (equivShrink _).symm).surjective.range_comp _).trans Subtype.range_coe))
  abs_map f x := Subtype.ext (Prod.ext
    (Set.range_comp f (x.2 ∘ Sum.inl)) (Set.range_comp f (x.2 ∘ Sum.inr)))

def LGame := QPF.Cofix F.{u}
...
theorem eq_of_bisim (r : LGame → LGame → Prop)
    (leftMoves : ∀ x y, r x y → ∃ e : leftMoves x ≃ leftMoves y, ∀ i, r i.1 (e i).1)
    (rightMoves : ∀ x y, r x y → ∃ e : rightMoves x ≃ rightMoves y, ∀ i, r i.1 (e i).1)
    (x y : LGame.{u}) (hxy : r x y) : x = y := by
  refine QPF.Cofix.bisim r (fun x y hxy => ?_) x y hxy
  obtain ⟨el, hel⟩ := leftMoves x y hxy
  obtain ⟨er, her⟩ := rightMoves x y hxy
  refine ⟨⟨(Set.range fun i => ⟨(i.1, (el i).1), hel i⟩,
    Set.range fun i => ⟨(i.1, (er i).1), her i⟩), inferInstance, inferInstance⟩, ?_, ?_⟩
  · apply Subtype.ext
    apply Prod.ext
    · apply (Set.range_comp _ _).symm.trans
      simp [Function.comp_def, LGame.leftMoves]
    · apply (Set.range_comp _ _).symm.trans
      simp [Function.comp_def, LGame.rightMoves]
  · apply Subtype.ext
    apply Prod.ext
    · apply (Set.range_comp _ _).symm.trans
      apply (Set.range_comp Subtype.val el).trans
      simp [LGame.leftMoves]
    · apply (Set.range_comp _ _).symm.trans
      apply (Set.range_comp Subtype.val er).trans
      simp [LGame.rightMoves]

@[ext]
protected theorem ext {x y : LGame.{u}}
    (leftMoves : x.leftMoves = y.leftMoves)
    (rightMoves : x.rightMoves = y.rightMoves) : x = y :=
  eq_of_bisim
...
private def T_coeval (x : Shrink.{u + 1} (T leftMoves rightMoves init)) :
    F (Shrink.{u + 1} (T leftMoves rightMoves init)) :=
  have x := (equivShrink _).symm x
