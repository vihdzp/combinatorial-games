...
theorem Nimber.Iio_of (n : ℕ) : Iio (∗n) = NatOrdinal.of '' Iio n := by aesop
...
theorem forall_leftMoves_nim_natCast {P : IGame → Prop} {n : ℕ} :
  ...
  rw [leftMoves_nim, Nimber.Iio_of, Ordinal.Iio_natCast]
  simp

@[game_cmp]
theorem forall_rightMoves_nim_natCast {P : IGame → Prop} {n : ℕ} :
    (∀ x ∈ rightMoves (nim (∗n)), P x) ↔ ∀ m < n, P (nim (∗m)) := by
  rw [rightMoves_nim, Nimber.Iio_of, Ordinal.Iio_natCast]
  simp
