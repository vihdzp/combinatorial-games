...
This is an impartial game, and in a strong sense, Nim represents the entire class of impartial
games: by the Sprague-Grundy theorem (proven in `Game.Impartial.Grundy`), every impartial game is
equivalent to a game of Nim.

Instead of taking `o : Ordinal` for `nim`, we take values in the type alias `Nimber`. This allows us
to more state results such as `nim (a + b) ≈ nim a + nim b` and `nim (a * b) ≈ nim a * nim b`.
-/
