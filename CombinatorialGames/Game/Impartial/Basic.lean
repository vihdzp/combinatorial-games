...
theorem impartial_def' {x : IGame} :
    x.Impartial ↔ -x ≈ x ∧ (∀ i ∈ x.moves_left, Impartial i) ∧ ∀ j ∈ x.moves_right, Impartial j := by
...
theorem mk {x : IGame} (h₁ : -x ≈ x)
    (h₂ : ∀ i ∈ x.moves_left, Impartial i) (h₃ : ∀ j ∈ x.moves_right, Impartial j) : Impartial x :=
...
protected instance neg (x : IGame) [Impartial x] : Impartial (-x) := by
  apply mk
  · simp
  on_goal 1 => rw [moves_left, moves_neg]
  on_goal 2 => rw [moves_right, moves_neg]
  all_goals
  · intro y hy
...
protected instance add (x y : IGame) [Impartial x] [Impartial y] : Impartial (x + y) := by
  ...
  on_goal 1 => rw [moves_left, moves_add]
  on_goal 2 => rw [moves_right, moves_add]
  all_goals
...
theorem fuzzy_left {y : IGame} (hy : y ∈ x.moves_left) : x ‖ y := by
  have := hx.of_mem_moves hy
  simpa using left_lf hy

theorem left_fuzzy {y : IGame} (hy : y ∈ x.moves_left) : y ‖ x :=
  (fuzzy_left hy).symm

theorem right_fuzzy {y : IGame} (hy : y ∈ x.moves_right) : y ‖ x := by
  have := hx.of_mem_moves hy
  simpa using lf_right hy

theorem fuzzy_right {y : IGame} (hy : y ∈ x.moves_right) : x ‖ y :=
  (right_fuzzy hy).symm

/-- This version is stated in terms of left moves of `x` and right moves of `y`. -/
theorem equiv_iff_forall_fuzzy :
    x ≈ y ↔ (∀ z ∈ x.moves_left, z ‖ y) ∧ (∀ z ∈ y.moves_right, x ‖ z) := by
...
theorem equiv_iff_forall_fuzzy' :
    x ≈ y ↔ (∀ z ∈ x.moves_right, z ‖ y) ∧ (∀ z ∈ y.moves_left, x ‖ z) := by
...
theorem fuzzy_iff_exists_equiv :
    x ‖ y ↔ (∃ z ∈ y.moves_right, x ≈ z) ∨ (∃ z ∈ x.moves_left, z ≈ y) := by
...
theorem equiv_zero : x ≈ 0 ↔ ∀ y ∈ x.moves_left, y ‖ 0 := by
  rw [equiv_iff_forall_fuzzy]; simp

/-- This version is stated in terms of right moves of `x`. -/
theorem equiv_zero' : x ≈ 0 ↔ ∀ y ∈ x.moves_right, y ‖ 0 := by
  rw [equiv_iff_forall_fuzzy']; simp

/-- This version is stated in terms of left moves of `x`. -/
theorem fuzzy_zero : x ‖ 0 ↔ ∃ y ∈ x.moves_left, y ≈ 0 := by
  rw [fuzzy_iff_exists_equiv]; simp

/-- This version is stated in terms of right moves of `x`. -/
theorem fuzzy_zero' : x ‖ 0 ↔ ∃ y ∈ x.moves_right, y ≈ 0 := by
  rw [fuzzy_iff_exists_equiv']; simp
...
theorem fuzzy_zero_of_forall_exists_moveLeft {y} (hy : y ∈ x.moves_left)
    (H : ∀ z ∈ y.moves_left, ∃ w ∈ x.moves_left, z ≈ w) : x ‖ 0 := by
...
theorem fuzzy_zero_of_forall_exists_moveRight {y} (hy : y ∈ x.moves_right)
    (H : ∀ z ∈ y.moves_right, ∃ w ∈ x.moves_right, z ≈ w) : x ‖ 0 := by
  rw [← neg_fuzzy_zero]
