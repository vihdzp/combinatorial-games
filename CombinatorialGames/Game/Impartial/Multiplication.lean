/-
Copyright (c) 2025 Fox Thomson. All rights reserved.
...
/-!
# Multiplication of weakly impartial games

We prove here that the product of weakly impartial games is weakly impartial, and that its Grundy
value equals the product of the Grundy values of its factors. This implies that game multiplication
is algebraically well-behaved on weakly impartial games, i.e. it respects equivalence, and there are
no zero divisors.
-/
...
protected instance mul (x y : IGame) [WeaklyImpartial x] [WeaklyImpartial y] :
    WeaklyImpartial (x * y) :=
  (mul' x y).1
...
variable {x y x₁ x₂ y₁ y₂ : IGame} [WeaklyImpartial x] [WeaklyImpartial y]
