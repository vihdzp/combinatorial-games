...
It's tempting to define `a * b` as the Grundy value of `nim a * nim b`, similarly to how we did for
addition. The problem is that `x * y ≈ 0 → x ≈ 0 ∨ y ≈ 0` is not true for general games, meaning
that we aren't able to build the `MulZeroClass` instance using this definition until much later. We
instead opt to prove the connection between the nimber product and the game product in
`CombinatorialGames.Game.Impartial.Multiplication`.

As for the nimber inverse, it's possible to show its existence implicitly via the simplest extension
theorems. Instead, we employ the explicit formula given in [On Numbers And Games][conway2001]
...
instance : MulZeroClass Nimber := by
  constructor
  all_goals
    simp_rw [← Nimber.le_zero]
    exact fun _ ↦ mul_le_of_forall_ne (by simp)
