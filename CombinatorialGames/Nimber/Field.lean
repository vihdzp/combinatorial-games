...
/-! ### Grundy value of product -/

private instance impartial_nim_mul_nim (a b : Nimber) : Impartial (nim a * nim b) := by
  apply Impartial.mk
  · rw [← neg_mul, neg_nim]
  · simp_rw [forall_moves_mul, forall_moves_nim, mulOption]
    intro _ _ c hc d hd
    have := impartial_nim_mul_nim c b
    have := impartial_nim_mul_nim a d
    have := impartial_nim_mul_nim c d
    infer_instance
termination_by (a, b)

/-- Nimber multipplication `a * b` is defined as the Grundy value of `nim a * nim b`. -/
instance : Mul Nimber where
  mul a b := Impartial.grundy (nim a * nim b)

namespace IGame.Impartial

theorem grundy_nim_mul_nim (a b : Nimber) : grundy (nim a * nim b) = a * b :=
  rfl
