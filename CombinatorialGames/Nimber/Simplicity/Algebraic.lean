...
theorem IsField.isRoot_leastNotSplit {x : Nimber} (h : IsField x) (h' : ¬ IsAlgClosed x) :
    (x.leastNotSplit.untop
      ((isAlgClosed_iff_leastNotSplit_eq_top h.toIsRing).not.1 h')).IsRoot x := by
