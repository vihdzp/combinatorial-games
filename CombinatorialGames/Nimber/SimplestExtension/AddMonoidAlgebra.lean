...
def toAddMonoidAlgebraIso : Nimber ≃+ hb.toSubfield[Ordinal] :=
  sorry
...
theorem toAddMonoidAlgebra_add (x y : Nimber) :
    toAddMonoidAlgebra b hb (x + y) = toAddMonoidAlgebra b hb x + toAddMonoidAlgebra b hb y := by
  rw [← ofAddMonoidAlgebra_inj b hb]
  sorry
