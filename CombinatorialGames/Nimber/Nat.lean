...
@[norm_cast]
theorem cast_pow (m : ℕ) : ∀ n : ℕ, ↑(m ^ n) = (m ^ n : Ordinal) := by
  simp [Ordinal.opow_natCast]

@[simp]
theorem two_two_pow (n : ℕ) : (2 : Ordinal) ^ (2 : Ordinal) ^ n = 2 ^ 2 ^ n := by
  norm_cast
...
theorem IsField.two_two_pow (n : ℕ) : IsField (∗(2 ^ 2 ^ n)) := by
  ...
  | zero => simp
  | succ n =>
    have hl := WithTop.untop_eq (leastNoRoots_two_two_pow n)
    generalize_proofs _ ht at hl
    convert (IsField.two_two_pow n).pow_degree_leastNoRoots ht _ (n := 2) using 1
...
theorem IsField.two_two_pow (n : ℕ) : IsField (∗(2 ^ 2 ^ n)) := by
  ...
  | succ n =>
    ...
    have he : Nat.cast (2 ^ 2 ^ (n + 1)) = (val (∗(2 ^ 2 ^ n))) ^ 2 := by
      rw [val_of, ← Ordinal.opow_natCast, Nat.pow_succ, Nat.pow_mul]
      simp
    rw [he]
    have ht : leastNoRoots (∗(2 ^ 2 ^ n)) ≠ ⊤ :=
...
private theorem algebraTrace_eq {m n : ℕ} (h : m ≤ n) :
  ∃ (hm : IsField (∗(2 ^ 2 ^ m))) (hn : IsField (∗(2 ^ 2 ^ n))),
    let algebra : Algebra hm.toSubfield hn.toSubfield :=
      Subfield.inclusion (Set.Iio_subset_Iio (two_two_pow_le h)) |>.toAlgebra
    ∀ x hx, Algebra.trace hm.toSubfield hn.toSubfield ⟨∗x, hx⟩ =
      ∗(x / 2 ^ (2 ^ n - 2 ^ m)) := by
  have hf : IsField (∗(2 ^ 2 ^ n)) := .two_two_pow n
  have hfm (m : ℕ) (hm : m < n) : IsField (∗(2 ^ 2 ^ m)) := .two_two_pow m
  ...
  have : Finite (hfm n (by simp)).toSubfield :=
    Set.Finite.to_subtype (mod_cast finite_Iio_natCast (2 ^ 2 ^ n))
  have : Finite hf.toSubfield :=
    Set.Finite.to_subtype (mod_cast finite_Iio_natCast (2 ^ 2 ^ (n + 1)))
  ...
  suffices ht : ua = ∗(x / 2 ^ (2 ^ n)) by
    ...
    rw [htr, Nat.div_div_eq_div_mul, ← Nat.pow_add,
      ...
      -/
  change (algebraMap (hfm n (by simp)).toSubfield hf.toSubfield ua).1 = ∗(x / 2 ^ (2 ^ n))
  set d := x / 2 ^ 2 ^ n
  set r := x % 2 ^ 2 ^ n
  ...
  clear_value d r
  let u2 : hf.toSubfield := ⟨∗(2 ^ 2 ^ n), two_two_pow_lt (by simp)⟩
  let d2 : (hfm n (by simp)).toSubfield := ⟨∗d, of.strictMono (Nat.cast_lt.2 hd)⟩
  let r2 : (hfm n (by simp)).toSubfield := ⟨∗r, of.strictMono (Nat.cast_lt.2 hr)⟩
  subst hdr
  ...
  have h22 : ∗(2 ^ (2 ^ n - 1)) < ∗(2 ^ 2 ^ n) := by
    simpa using coeff_polynomial_lt n 0
  have poly : minpoly (hfm n (by simp)).toSubfield u2 =
      X ^ 2 + X + C ⟨∗(2 ^ (2 ^ n - 1)), h22⟩ := by
    have hl := leastNoRoots_eq n
    have ht : (∗(2 ^ 2 ^ n)).leastNoRoots ≠ ⊤ :=
      fun h => WithTop.coe_ne_top (hl.symm.trans h)
    have uk : (hfm n (by simp)).embed
        ((∗(2 ^ 2 ^ n)).leastNoRoots.untop ht) (coeff_leastNoRoots_lt ht) =
        X ^ 2 + X + C ⟨∗(2 ^ (2 ^ n - 1)), h22⟩ := by
    ...
    · ext
      ...
      change eval (∗(2 ^ 2 ^ n)) (map (hfm n (by simp)).toSubfield.subtype _) = 0
...
theorem leastNoRoots_eq (n : ℕ) : leastNoRoots (∗(2 ^ 2 ^ n)) =
    WithTop.some (C 1 * X ^ 2 + C 1 * X + C (∗(2 ^ (2 ^ n - 1)))) := by
  have hf := IsField.two_two_pow n
  ...
  · apply le_of_forall_lt_imp_ne
    ...
    have ht : (∗(2 ^ 2 ^ n)).leastNoRoots ≠ ⊤ := fun h => WithTop.coe_ne_top (heq.trans h)
    ...
    · rw [lt_one_iff_zero] at hb
      subst hb
      have hc : c < ∗(2 ^ 2 ^ n) := by simpa using coeff 0
    ...
    · rw [lt_one_iff_zero] at ha
      subst ha
      have hb : b < ∗(2 ^ 2 ^ n) := by simpa using coeff 1
      have hc : c < ∗(2 ^ 2 ^ n) := by simpa using coeff 0
