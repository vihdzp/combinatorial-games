...
attribute [-simp] LinearOrderedField.inducedOrderRingHom_toFun
...
noncomputable instance : LinearOrder (FiniteResidueField K) :=
  @Quotient.instLinearOrderOfDecidableRelEquiv _ _ _ ordConnected_preimage_mk' (Classical.decRel _)
...
theorem standardPart_ne_zero {x : K} (h : mk x = 0) : standardPart x ≠ 0 := by
